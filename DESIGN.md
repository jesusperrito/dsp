...
### Article pager
Each content section shows one full-width article at a time instead of a horizontally-scrollable row of fixed-width cards — for presentation, a single focal piece per section reads better than a shelf to browse. `.card-pager > .card-pager-track` holds every article in the section at `flex: 0 0 100%`; `main.js` slides the track via `transform: translateX()` in response to prev/next arrow clicks (`.card-pager-controls`), clamped at both ends (no wrap-around) with a "N / total" counter. The card itself (cream paper, ink border) is unchanged apart from now being full-width instead of a fixed max-width. Every article across every section is fully written (sourced from the studio's `Times Of The Wild.pdf`) — an earlier pass left everything but the first article per section as an undressed "wireframe" placeholder, but that undersold the format, so all 17 articles are dressed now.
...
- **Link type is 10.5px** (down from an earlier 12px, with tighter 12px horizontal padding and 0.04em tracking) so more of the 12 section tabs fit before horizontal scroll kicks in — the contrast ratio math (0.65 opacity ink-on-cream, 5.5:1) doesn't depend on font-size below the WCAG "large text" threshold, so this didn't need re-checking against the touch-target/contrast rules below.
...
- **Face-in-holder sizing uses `object-fit: cover` + per-animal `object-position`**, not a manual scale/offset. The 8 face SVGs have wildly different internal aspect ratios and padding (a tall narrow crane neck vs. a wide hippo head vs. a mostly-empty-canvas oryx). An early pass measured each face's content bounding box in a normalized *square* canvas and drove a hand-rolled scale/position from that — but `cover` crops using the image's own *natural* aspect ratio, so a mismatched measurement space silently cropped the crane's portrait to nothing. `object-position` values are the content's center measured as a % of that image's own real width/height (not a normalized square), which is what `cover` actually needs; `.moodcast-face--crane`/`--ostrich` additionally get a small `transform: scale()` since their content is a thin sliver even within their own frame.
...
- **Don't** transition `left`/`right`/`top`/`bottom` directly on an element that can switch between opposite edges (e.g. the paw trinket moving between the left and right side of the screen). Switching which physical property is animated mid-sequence caused a visible full-screen drag in at least one real environment. Animate a single `transform: translate()` instead, with the untransitioned `left`/`top`/`right`/`bottom` only setting the static "dock" position — see `.paw-trinket` in `styles.css`.
- **Don't** size a face/photo image inside a circular crop with `object-fit: contain`/manual scale math measured against a square-normalized canvas, then apply it via `object-fit: cover`. Measure content position as a % of the image's own natural (unnormalized) width/height and drive the crop with `object-position` — `object-fit: cover` crops using the image's real aspect ratio, and mixing coordinate spaces silently cropped the Wild Moodcast's crane portrait to nothing.
